...
namespace QB_App.BillProcessing
{
    public class InvoiceAdder
    {
        ...
        public InvoiceAdder(ref Dictionary<string, Account> Accounts, Client client)
        {
            this.client = client;
        ...
        {
            XmlElement expenseLine = inputXMLDoc.CreateElement("InvoiceLineAdd");
            /*
        ...
        public string InsertInvoice(ref QuickbooksQueries quickbooksQueries, Bill bill)
        {
            ...
            // Mark as query type
            XmlElement InvoicesAddRq = inputXMLDoc.CreateElement("InvoiceAddRq");
            qbXMLMsgsRq.AppendChild(InvoicesAddRq);
            XmlElement invoiceXML = inputXMLDoc.CreateElement("InvoiceAdd");

            InvoicesAddRq.AppendChild(invoiceXML);

            XmlElement vendorRef = inputXMLDoc.CreateElement("CustomerRef");
            XmlElement vendorListID = inputXMLDoc.CreateElement("ListID");
            vendorListID.InnerText = client.listId;
            ...
            bool billExists = quickbooksQueries.QueryExistingInvoice(billDate, bill.uuid);
            ...
            else
            {
                ...
                refNumber.InnerText = folioAndSeries.Substring(0, Math.Min(11, folioAndSeries.Length));
                ...
                /*toma los datos del documento y los pasa a exchangerate, en especial el tipo de cambio*/
                if (bill.moneda != "mxn")
                {
                ...
                {
                    QuicbooksResponse qbResultUpdateInvoice = new QuicbooksResponse(updateResponse, "InvoiceAddRs");
                    if (qbResultUpdateInvoice.GetNumberOfResulst() > 0)
                    {
                        ...
                        {
                            return "No se puedo agregar factura " + bill.uuid + " cliente " + client.nombre +" " + qbResultUpdateInvoice.lastMessage;
                        }
                    }
                    else
                    {
                        return "Quickbooks regresó una respuesta vacía al agregar factura " + bill.uuid + " cliente " + client.nombre;
                    }
                }
                else
                {
                    return "Se produjo un error al agregar insertar factura " + bill.uuid + " cliente " + client.nombre + ": " + quickbooksQueries.lastError;
                }
            }
        }
    }
